    ...
    def fit(
        ...
        for epoch in range(self.epochs):
            ...
            for batch_X, batch_y in train_loader:
                ...
                batch_X = batch_X.unsqueeze(0)  # [1, batch_size, features]
                
                # Embedding and positional encoding
                embedded = self.input_embedding(batch_X)  # [1, batch_size, d_model]
                embedded = self.pos_encoder(embedded)
                
                # Transformer (using embedded as both src and tgt for autoregressive)
                output = self.transformer(embedded, embedded)  # [1, batch_size, d_model]
                
                # Output layer
                predictions = self.output_layer(output.squeeze(0))  # [batch_size, forecast_horizon]
            ...
            if X_val is not None and y_val is not None:
                ...
                with torch.no_grad():
                    ...
                    embedded = self.input_embedding(X_val_tensor)
                    embedded = self.pos_encoder(embedded)
